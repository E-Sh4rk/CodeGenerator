...
let parsed_ast_to_arm ~optimize env lst =
  try
...
let parsed_content_to_arm fmt ~optimize env (headers, lst) =
  let env =
    Preprocess.env_from_headers fmt headers |>
    Preprocess.concat_env env in
  parsed_ast_to_arm ~optimize env lst
